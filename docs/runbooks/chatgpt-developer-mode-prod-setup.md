...
- Dedicated owner account + ResearchAgent for testing
...
- Auth0 API identifier (audience): `https://api.analog-research.org`
- MCP server URL (recommended): `https://api.analog-research.org/v1/mcp/chatgpt` (rewrites to `/api/v1/mcp/chatgpt`)
- Resource metadata `resource` matches the Auth0 API identifier (use `https://api.analog-research.org`)
...
Set these on the `@analogresearch/web` site in Netlify `production` context:

- `MCP_OAUTH_ENABLED=true`
- `MCP_OAUTH_PROVIDER=auth0`
- `MCP_OAUTH_ISSUER=https://<tenant>.auth0.com`
- `MCP_OAUTH_AUDIENCE=https://api.analog-research.org`
- `MCP_OAUTH_RESOURCE=https://api.analog-research.org` (must match Auth0 API identifier for `resource` parameter)
- `MCP_OAUTH_SCOPES_READ=analogresearch.read`
- `MCP_OAUTH_SCOPES_WRITE=analogresearch.write`
- `AUTH0_MCP_LINK_CLIENT_ID=<client id>`
- `AUTH0_MCP_LINK_CLIENT_SECRET=<client secret>`

Optional but recommended for canonical URLs:
- `NEXT_PUBLIC_APP_URL=https://analog-research.org`
- `NEXT_PUBLIC_SITE_URL=https://analog-research.org`
...
Expected:
- `resource` is `https://api.analog-research.org`
- `authorization_servers` includes your Auth0 issuer
- `scopes_supported` includes `analogresearch.read` and `analogresearch.write`
...
- `_meta["mcp/www_authenticate"]` includes `resource_metadata="https://api.analog-research.org/.well-known/oauth-protected-resource"`
...
1. Create new app
2. MCP server URL: `https://api.analog-research.org/v1/mcp/chatgpt`
