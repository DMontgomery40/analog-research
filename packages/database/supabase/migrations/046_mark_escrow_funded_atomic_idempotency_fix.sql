-- Align escrow-funding RPC idempotency conflict target with partial unique index.
...
BEGIN
  ...
  IF v_booking.escrow_status = 'pending' THEN
    ...
    IF FOUND THEN
      ...
      ON CONFLICT (booking_id) WHERE (type = 'escrow_fund') DO NOTHING;
  ...
  SELECT b.status::TEXT, b.escrow_status::TEXT
  INTO v_current_status, v_current_escrow_status
  FROM bookings b
  WHERE b.id = p_booking_id;

  transitioned := v_transitioned;
  booking_status := COALESCE(v_current_status, '');
  escrow_status := COALESCE(v_current_escrow_status, '');
  RETURN NEXT;
END;
$$;
