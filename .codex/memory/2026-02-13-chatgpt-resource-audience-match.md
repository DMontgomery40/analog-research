...
## Fix
Set `MCP_OAUTH_RESOURCE` to the Auth0 API identifier (same value as `MCP_OAUTH_AUDIENCE`, e.g., `https://api.analoglabor.com`). Redeploy so `resource` in metadata matches.
