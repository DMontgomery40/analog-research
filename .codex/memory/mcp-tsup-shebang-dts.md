...
Fix: remove the shebang from `src/index.ts` and add it back to `dist/index.js` in a post-build step (also `chmod +x`), e.g. `packages/analoglabor-mcp/scripts/add-shebang.mjs`.
