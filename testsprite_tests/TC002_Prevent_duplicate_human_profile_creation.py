...
async def run_test():
    ...
    try:
        ...
        # -> Open the Login page by clicking the Login link (index 83).
        frame = context.pages[-1]
        # Click element
        elem = frame.locator('xpath=html/body/div[2]/nav/div/div[2]/a[1]').nth(0)
        await page.wait_for_timeout(3000); await elem.click(timeout=5000)
        
        # -> Ensure the login form is visible by clicking the Login link again if necessary (index 83). After the login form appears, fill email and password with the regular user credentials and submit to authenticate.
        frame = context.pages[-1]
        # Click element
        elem = frame.locator('xpath=html/body/div[2]/nav/div/div[2]/a[1]').nth(0)
        await page.wait_for_timeout(3000); await elem.click(timeout=5000)
        ...
        # -> Wait for login to complete (if still processing), then navigate to /api/v1/humans to trigger/inspect the API response (expect 409 Conflict).
